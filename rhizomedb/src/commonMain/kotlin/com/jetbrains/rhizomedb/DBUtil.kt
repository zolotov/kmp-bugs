...
internal fun Q.displayEntity(eid: EID): String =
    "${entityType(eid)?.let { typeEID -> displayEntityType(typeEID) }}[$eid]"
...
fun <T : Any> DbContext<Q>.getOne(eid: EID, attr: Attribute<T>, throwIfNoEntity: Boolean = false): T? = impl.getOne(
    eid,
    attr,
    throwIfNoEntity
)
...
internal fun Q.entityTypesByName(name: String): Set<EID> =
    queryIndex(
        IndexQuery.LookupMany(
            EntityType.Name.attr as Attribute<String>,
            name
        )
    ).mapTo(HashSet()) { datom -> datom.eid }
...
internal fun Q.throwEntityDoesNotExist(
    eid: EID,
...
internal fun DbContext<Q>.assertRequiredAttrs(entityEID: EID, entityTypeEID: EID): Nothing? {
    ...
    return when {
        missing.isNotEmpty() -> throw TxValidationException(missing.map { m -> message(m) }
