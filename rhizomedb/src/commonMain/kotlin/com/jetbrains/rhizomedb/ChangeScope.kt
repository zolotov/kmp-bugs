...
interface ChangeScope {
    ...
    operator fun <E : Entity, V : Any> E.set(attribute: Attributes<E>.Required<V>, value: V): Unit =
        set_cs(attribute, value)

    operator fun <E : Entity, V : Any> E.set(attribute: Attributes<E>.Optional<V>, value: V?): Unit =
        set_cs(attribute, value)

    operator fun <E : Entity, V : Any> E.set(attribute: Attributes<E>.Many<V>, values: Set<V>): Unit =
        set_cs(attribute, values)
    ...
    fun <T> withDefaultPart(partition: Int, body: ChangeScope.() -> T): T =
        com.jetbrains.rhizomedb.withDefaultPart(partition, body)
    ...
    fun <E : Entity, V : Any> EntityType<E>.upsert(
        attribute: EntityAttribute<E, V>,
        value: V,
        builder: EntityBuilder<E>
    ): E = upsert_cs(attribute, value, builder)
}
        ...
        cs.context.run {
            builder.build(object : EntityBuilder.Target<E> {
                ...
                override fun <V : Any> set(attribute: Attributes<in E>.Many<V>, values: Set<V>) =
                    entity.set_cs(attribute, values)
            })
        }
    }
...
fun <E : Entity> EntityType<E>.new(builder: EntityBuilder<E> = EntityBuilder {}): E = let { entityType ->
    ...
    val eid = cs.context.impl.createEntity(
        pipeline = cs.context,
        entityTypeEid = entityType.eid,
